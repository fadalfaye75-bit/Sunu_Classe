      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 md:mb-10 gap-4">
        <div>
          <h1 className="text-3xl md:text-4xl font-black text-[#2D1B0E] flex items-center gap-3 tracking-tight">
            <span className="bg-purple-100 border-2 border-purple-300 p-2 rounded-xl shadow-[4px_4px_0_#9333EA]"><Vote className="text-purple-700 w-6 h-6 md:w-8 md:h-8" /></span>
            Sondages
          </h1>
          <p className="text-[#5D4037] mt-2 font-bold text-base md:text-lg">La voix de la classe compte.</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-6 md:gap-8">
        {polls.length === 0 && (
          <div className="text-center py-20 bg-white rounded-3xl border-4 border-dashed border-[#D6C0B0]">
             <Vote className="w-16 h-16 md:w-20 md:h-20 mx-auto mb-4 opacity-20 text-[#2D1B0E]" />
        ...
        {polls.map(poll => {
          ...
          return (
            <div key={poll.id} className="bg-white rounded-3xl shadow-sm border-2 border-[#D6C0B0] p-5 md:p-8 hover:border-purple-300 transition group">
              <div className="flex flex-col md:flex-row justify-between items-start mb-6 md:mb-8 gap-4">
                <div className="flex-1 w-full">
                   <h3 className="text-xl md:text-2xl font-black text-[#2D1B0E] leading-tight">{poll.question}</h3>
                ...
                {(user?.role === Role.RESPONSIBLE || user?.role === Role.ADMIN) && (
                  <div className="flex gap-2 w-full md:w-auto">
                    <button onClick={() => openEdit(poll)} className="flex-1 md:flex-none justify-center text-indigo-700 bg-indigo-50 hover:bg-indigo-100 p-3 rounded-xl transition border border-indigo-100 active:scale-95">
                      <Pencil className="w-5 h-5" />
                    </button>
                    <button onClick={() => deletePoll(poll.id)} className="flex-1 md:flex-none justify-center text-red-700 bg-red-50 hover:bg-red-100 p-3 rounded-xl transition border border-red-100 active:scale-95">
              ...
              <div className="grid md:grid-cols-2 gap-8 md:gap-10">
                ...
                {/* Results Section */}
                <div className="min-h-[220px] flex flex-col justify-center items-center bg-[#FFF8F0] rounded-2xl border-2 border-[#EFEBE9] p-4 md:p-6">
                  {!canViewResults ? (
            ...
            <div className="overflow-y-auto p-5 md:p-8 bg-white">
              <form onSubmit={handleSubmit} className="space-y-5 md:space-y-6">
                <div>
                  ...
                  <input required type="text" value={question} onChange={e => setQuestion(e.target.value)} className="w-full bg-[#FFF8F0] border-2 border-[#D6C0B0] rounded-xl p-3 md:p-4 focus:border-purple-500 focus:bg-white outline-none transition font-bold text-[#2D1B0E]" />
                </div>
                <div>
                  <label className="block text-sm font-black text-[#2D1B0E] mb-2 uppercase">Options (séparées par virgule)</label>
                  <textarea required value={optionsStr} onChange={e => setOptionsStr(e.target.value)} className="w-full bg-[#FFF8F0] border-2 border-[#D6C0B0] rounded-xl p-3 md:p-4 focus:border-purple-500 focus:bg-white outline-none transition font-bold text-[#2D1B0E]" placeholder="Option 1, Option 2, Option 3" rows={3}/>
                  {editingId && <p className="text-xs text-red-500 mt-2 font-bold">⚠️ Modifier les options réinitialise les votes.</p>}
                </div>
                
                <div className="flex items-center gap-3 p-3 md:p-4 bg-[#FFF8F0] rounded-xl border-2 border-[#D6C0B0] cursor-pointer hover:border-[#A1887F] transition active:bg-orange-50">
                   <input 
                     type="checkbox" 
                     id="anon"
                     checked={isAnonymous}
                   ...
                   />
                   <label htmlFor="anon" className="text-sm text-[#5D4037] flex items-center gap-2 cursor-pointer font-bold select-none uppercase tracking-wide flex-1">
